...
public class EntityParatiku extends EntityDivineTameable {
    private static final TargetingConditions RESTING_TARGETING = TargetingConditions.forCombat().range(4)/*.allowSameTeam()*/;
    //private BlockPos spawnPosition;
    public EntityParatiku(EntityType<? extends EntityParatiku> type, Level level) {
        super(type, level, 1F);
        if(!level.isClientSide) setIsParatikuHanging(true);
    }
    ...
    public boolean getIsParatikuHanging() {return AttachmentRegistry.HANGING.get(this);}
    public void setIsParatikuHanging(boolean isHanging) {AttachmentRegistry.HANGING.setSilent(this, isHanging);}
    @Override public void tick() {
        super.tick();
        if(getIsParatikuHanging()) {
            setDeltaMovement(0, 0, 0);
            setPosRaw(getX(), (double)Mth.floor(getY()) + 1 - getBbHeight(), getZ());
        } else setDeltaMovement(getDeltaMovement().x, getDeltaMovement().y * .6, getDeltaMovement().z);
    }
    @Override protected void customServerAiStep() {
        ...
        if(getIsParatikuHanging()) {
            ...
            if(getTarget() != null) {
                ...
                if(random.nextInt(30) == 0 || getTarget().blockPosition().closerToCenterThan(position(), 2))
                    moveTo(getX() + random.nextInt(7) - random.nextInt(7), getY() + random.nextInt(6) - 2, getZ() + random.nextInt(7) - random.nextInt(7));
                double d2 = getTarget().blockPosition().getX() + .5 - getX();
                double d0 = getTarget().blockPosition().getY() + .1 - getY();
                double d1 = getTarget().blockPosition().getZ() + .5 - getZ();
                ...
                float f = (float)(Mth.atan2(vector3d1.z, vector3d1.x) * Mth.RAD_TO_DEG) - 90;
    ...
    @Override public boolean isFood(ItemStack item) {return item.is(TagRegistry.PET_FOODS_PARATIKU);}
    @Override protected boolean isTamingFood(ItemStack item) {return item.is(TagRegistry.PET_TAMING_FOODS_PARATIKU);}
