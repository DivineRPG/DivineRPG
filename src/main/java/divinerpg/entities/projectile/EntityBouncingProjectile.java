...
public class EntityBouncingProjectile extends EntityShooterBullet {
    ...
    public EntityBouncingProjectile(EntityType<? extends ThrowableProjectile> type, Player player, Level world, BulletType bulletType) {
    ...
    @Override public void onHitEntity(EntityHitResult result) {
        if(result.getEntity() instanceof LivingEntity entity && entity != thrower) {
            entity.hurt(damageSources().indirectMagic(this, getOwner()), getBulletType().getDamage());
            discard();
        }
    }
