...
public class Tomato extends DivineThrownItem {
    public Tomato(EntityType<? extends ThrowableItemProjectile> entityType, Level level) {super(entityType, level);}
    ...
    @Override public void handleEntityEvent(byte id) {
        if(id == 3) for(int i = 0; i < 8; ++i) level().addParticle(new ItemParticleOption(ITEM, getItem()), getX(), getY(), getZ(), (random.nextFloat() - .5) * .08, (random.nextFloat() - .5) * .08, (random.nextFloat() - .5) * .08);
    }
    @Override protected Item getDefaultItem() {return tomato.get();}
}