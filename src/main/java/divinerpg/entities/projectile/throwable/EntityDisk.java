...
public class EntityDisk extends DivineThrownItem {
    ...
    @Override protected void onHitBlock(BlockHitResult result) {
        if(getOwner() == null) super.onHitBlock(result);
            ...
            if(result.getDirection() == Direction.DOWN || result.getDirection() == Direction.UP) lerpMotion(x * .6, y * -.6, z * .6);
            else if(result.getDirection() == Direction.EAST || result.getDirection() == Direction.WEST) lerpMotion(x * -.6, y * .6, z * .6);
            else if(result.getDirection() == Direction.NORTH || result.getDirection() == Direction.SOUTH) lerpMotion(x * .6, y * .6, z * -.6);
        }
    }
