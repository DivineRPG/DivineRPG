...
public class EntityEnderTripletsFireball extends DivineFireball {
    ...
    @Override public void tick() {
        super.tick();
        if(level().isClientSide()) level().addParticle(PORTAL, xo + (random.nextDouble() - random.nextDouble()) / 6, yo + .5 + (random.nextDouble() - random.nextDouble()) / 6, zo + (random.nextDouble() - random.nextDouble()) / 6, 0, 0, 0);
