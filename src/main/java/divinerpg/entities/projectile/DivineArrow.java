...
public abstract class DivineArrow extends AbstractArrow {
     ...
     */
    public float getArrowPower() {return 2;}
    ...
    @Override protected void doPostHurtEffects(LivingEntity living) {
        ...
        if(potioncontents.potion().isPresent()) for(MobEffectInstance mobeffectinstance : potioncontents.potion().get().value().getEffects()){
            if(mobeffectinstance.getEffect().value().isInstantenous()) mobeffectinstance.getEffect().value().applyInstantenousEffect(getOwner(), this, living, mobeffectinstance.getAmplifier(), 1);
            ...
            if(mobeffectinstance1.getEffect().value().isInstantenous()) mobeffectinstance1.getEffect().value().applyInstantenousEffect(getOwner(), this, living, mobeffectinstance1.getAmplifier(), 1);
