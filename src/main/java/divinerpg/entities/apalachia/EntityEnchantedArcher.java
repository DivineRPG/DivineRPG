...
public class EntityEnchantedArcher extends EntityDivineMonster implements RangedAttackMob {
    public EntityEnchantedArcher(EntityType<? extends EntityEnchantedArcher> type, Level worldIn) {super(type, worldIn);}
    ...
    @Override public void performRangedAttack(LivingEntity target, float distanceFactor) {
        if(isAlive() && getTarget() != null) {
            ...
            abstractarrow.shoot(d0, d1 + d3 * .2, d2, 1.6F, 14F - (level().getDifficulty().getId() << 2));
