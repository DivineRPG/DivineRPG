...
public class EntityHelio extends EntityDivineMonster {
    ...
    public boolean canAttack(LivingEntity target) {
        if(super.canAttack(target)) {
            target.setDeltaMovement(getDeltaMovement().x + Mth.sin(getXRot() * (float)Math.PI / 180.0F) * 7.0F * 0.5F, getDeltaMovement().y + 0.4D, getDeltaMovement().z + Mth.cos(getXRot() * (float)Math.PI / 180.0F) * 7.0F * 0.5F);
            //this.motionX *= 0.6D;
            //this.motionZ *= 0.6D;
            return true;
        }
