...
public class EntityKobblin extends EntityDivineMonster implements RangedAttackMob {
    ...
    @Override public boolean isInvulnerableTo(ServerLevel level, DamageSource source) {return isEmerging() || super.isInvulnerableTo(level, source);}
    ...
    @Override public @Nullable SpawnGroupData finalizeSpawn(ServerLevelAccessor level, DifficultyInstance difficulty, EntitySpawnReason spawnType, @Nullable SpawnGroupData spawnGroupData) {
        if(spawnType == EntitySpawnReason.TRIGGERED && level.getBlockState(getOnPos()).is(BlockTags.DIRT)) setPose(Pose.EMERGING);
    ...
    @Override public void performRangedAttack(LivingEntity livingEntity, float v) {
        if(isAlive() && getTarget() != null && !level().isClientSide()) {
            DivineThrownItem shot = EntityRegistry.THROWN_ITEM.get().create(level(), EntitySpawnReason.EVENT);
