...
public class EntityKobblin extends EntityDivineMonster implements RangedAttackMob {
    ...
    protected void registerGoals() {
        ...
        goalSelector.addGoal(3, new KobblinRangedAttackGoal(1.25, 40, 60, 20F));
    ...
    public void performRangedAttack(LivingEntity livingEntity, float v) {
        if(isAlive() && getTarget() != null && !level().isClientSide) {
            ...
            double d1 = getTarget().getY(0.3333333333333333) - shot.getY() - 0.25;
    ...
    class KobblinRangedAttackGoal extends Goal {
        ...
        public boolean canContinueToUse() {
            return canUse() || target.isAlive() && !getNavigation().isDone();
        }
        public void stop() {
            target = null;
            seeTime = 0;
        ...
        public void tick() {
            ...
            if(attackTime == 10) setPose(Pose.SHOOTING);
                ...
                if(!hasSight) return;
                performRangedAttack(target, Mth.clamp((float)Math.sqrt(d0) / attackRadius, 0.1F, 1F));
