...
public class EntityWhale extends EntityDivineWaterMob {
    ...
    @Override public void tick() {
        ...
        else if(isInWaterOrBubble()) for(int i = 0; i < 5; i++) level().addParticle(SPLASH.get(), getX() + random.nextDouble() * .1 - .05, getY(), getZ() + random.nextDouble() * .1 - .05, random.nextDouble() * .5 - .25, random.nextDouble() * 2 + .5, random.nextDouble() * .5 - .25);
    }
