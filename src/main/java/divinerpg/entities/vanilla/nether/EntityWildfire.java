...
public class EntityWildfire extends EntityDivineMonster implements RangedAttackMob {
    ...
    @Override public @Nullable SpawnGroupData finalizeSpawn(ServerLevelAccessor level, DifficultyInstance difficulty, EntitySpawnReason type, @Nullable SpawnGroupData data) {
    ...
    @Override public void tick() {
        super.tick();
        getItemBySlot(MAINHAND).inventoryTick(level(), this, MAINHAND);
    }
    ...
    public static class LookingRangedBowAttackGoal<T extends Mob & RangedAttackMob> extends RangedBowAttackGoal<T> {
        ...
        public LookingRangedBowAttackGoal(T mob) {
            ...
            this.mob = mob;
            lookAtContext = TargetingConditions.forNonCombat().range(32).selector((target, level) -> EntitySelector.notRiding(mob).test(target));
        }
        @Override
        public void tick() {
            super.tick();
