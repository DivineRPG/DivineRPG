...
public class EntityKaros extends EntityDivineBoss {
                ...
                case 0:
                    ...
                    if (!this.level().isClientSide()) {
                        ...
                        for (Player p : players) {
                            p.sendSystemMessage(LocalizeUtils.getClientSideTranslation("message.karos.explosion"));
                        }
                    }
                    break;
                case 1:
    ...
    @Override
    public boolean hurtServer(ServerLevel level, DamageSource source, float par2) {
        ...
        if (!hasLoadedBlocks) {
            if (!this.level().isClientSide()) {
                ...
                for (Player p : players) {
                    ...
                    p.sendSystemMessage(LocalizeUtils.getClientSideTranslation("message.karos.game"));
                    p.sendSystemMessage(LocalizeUtils.getClientSideTranslation("message.karos.begin"));
                }
            }
            for (int x = -40; x < 40; x++) {
                for (int y = -5; y < 20; y++) {
...
//        int s = this.random.nextInt(4);
        ...
        level().getEntitiesOfClass(Player.class, this.getBoundingBox()
                ...
                .forEach(x -> x.sendSystemMessage(LocalizeUtils.getClientSideTranslation(langKey)));
