...
public class EntityWreck extends EntityDivineBoss {
    ...
    public void manageAbilities() {
        ...
        if (this.getAbility() == FREEZE) {
            if (player != null) {
                player.addEffect(new MobEffectInstance(MobEffects.SLOWNESS, 100, 5));
                this.setAbility(DEFAULT);
                abilityTimer = 100;
            }
        }
        ...
        for (Entity entity : list) {
            if (entity instanceof Player player) {
                ...
                switch (this.getAbility()) {
                    case CHARGE:
                        player.sendSystemMessage(LocalizeUtils.getClientSideTranslation("message.wreck.charge"));
                        break;
                    case PULL:
                        this.playSound(SoundRegistry.FEEL_SOUL_ARKSIANE.get(), 1.0F, 1.0F);
                        if (!level().isClientSide()) {
                            player.sendSystemMessage(LocalizeUtils.getClientSideTranslation("message.wreck.pull"));
                        }
                        break;
                    case FIRE:
                        player.sendSystemMessage(LocalizeUtils.getClientSideTranslation("message.wreck.fire"));
                        break;
                    case FREEZE:
                        this.playSound(SoundRegistry.STOP_AT_ONCE.get(), 1.0F, 1.0F);
                        if (!level().isClientSide()) {
                            player.sendSystemMessage(LocalizeUtils.getClientSideTranslation("message.wreck.freeze"));
                        }
                        break;
                    case SPEED:
                        this.playSound(SoundRegistry.WRECK_SPEED.get(), 1.0F, 1.0F);
                        if (!level().isClientSide()) {
                            player.sendSystemMessage(LocalizeUtils.getClientSideTranslation("message.wreck.speed"));
                        }
                        break;
                    case EXPLOSIONS:
                        this.playSound(SoundRegistry.EXPLOSIONS.get(), 1.0F, 1.0F);
                        if (!level().isClientSide()) {
                            player.sendSystemMessage(LocalizeUtils.getClientSideTranslation("message.wreck.explosion"));
                        }
                        break;
                    case STRENGTH:
                        this.playSound(SoundRegistry.WRECK_STRENGTH.get(), 1.0F, 1.0F);
                        if (!level().isClientSide()) {
                            player.sendSystemMessage(LocalizeUtils.getClientSideTranslation("message.wreck.strength"));
                        }
                        break;
                    default:
                        break;
    ...
    public void tick() {
        ...
        if (!level().isClientSide() && !loaded) {
            ...
            for (Player p : players) {
                p.sendSystemMessage(LocalizeUtils.getClientSideTranslation("message.wreck.run"));
                p.sendSystemMessage(LocalizeUtils.getClientSideTranslation("message.wreck.smell"));
            }
    ...
    public boolean doHurtTarget(ServerLevel level, Entity entity) {
        ...
        boolean var4 = entity.hurtServer(level, entity.level().damageSources().mobAttack(this), amount);
        ...
        if (var4) {
            ...
            if (knockback > 0) {
                entity.setDeltaMovement(-Mth.sin(getXRot() * (float) Math.PI / 180.0F) * knockback * 0.5F, 0.1D, Mth.cos(getYRot() * (float) Math.PI / 180.0F) * knockback * 0.5F);
            ...
            if(var5 > 0) entity.igniteForSeconds(var5 * 4);
        }
    ...
    public void performRangedAttack() {
        ...
        switch (this.getAbility()) {
            case BOUNCE:
                EntityBouncingProjectile projectile = EntityRegistry.BOUNCING_PROJECTILE.get().create(level(), EntitySpawnReason.EVENT);
            ...
            case SPEED:
                if (this.abilityTimer % 5 == 0) {
                    ThrowableProjectile shot = EntityRegistry.WRECK_SHOT.get().create(level(), EntitySpawnReason.EVENT);
            ...
            case EXPLOSIONS:
                if ((this.abilityTimer % 40) == 0) {
                    ThrowableProjectile explosiveShot = EntityRegistry.WRECK_BOMB.get().create(level(), EntitySpawnReason.EVENT);
            ...
            case STRENGTH:
                if ((this.abilityTimer % 40) == 0) {
                    DivineThrowableProjectile strengthShot = EntityRegistry.WRECK_SHOT.get().create(level(), EntitySpawnReason.EVENT);
