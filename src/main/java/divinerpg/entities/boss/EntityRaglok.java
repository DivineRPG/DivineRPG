...
public class EntityRaglok extends EntityDivineBoss {
    ...
    public void tick() {
        ...
        if (!loaded && !this.level().isClientSide()) {
            ...
            for (Player p : players) {
                p.sendSystemMessage(LocalizeUtils.getClientSideTranslation("message.raglok.dare"));
            }
    ...
    public void manageAbilities() {
        ...
        if (ability == SLOW) {
            if (player != null) {
                player.addEffect(new MobEffectInstance(MobEffects.SLOWNESS, 10, 1));
            }
        }
        if (ability == LIGHTNING) {
            if (abilityCooldown % 40 == 0 && player != null) {
                LightningBolt ent = new LightningBolt(EntityTypes.LIGHTNING_BOLT, level());
        ...
        if (ability == BOMBS) {
            if (abilityCooldown % 30 == 0 && player != null) {
                ...
                for (int i = 0; i < 4; i++) {
                    ...
                    ThrowableProjectile var2 = EntityRegistry.RAGLOK_BOMB.get().create(level(), EntitySpawnReason.EVENT);
                    var2.setOwner(this);
                    var2.moveOrInterpolateTo(new Vec3(player.xo, player.yo + 5, player.zo));
                ...
                switch (ability) {
                    case LIGHTNING:
                        ...
                        if (!level().isClientSide()) {
                            player.sendSystemMessage(LocalizeUtils.getClientSideTranslation("message.raglok.think"));
                            player.sendSystemMessage(LocalizeUtils.getClientSideTranslation("message.raglok.great"));
                        }
                        break;
                    case BLIND:
                        this.playSound(SoundRegistry.RAGLOK_DARK.get(), 1.0F, 1.0F);
                        if (!level().isClientSide()) {
                            player.sendSystemMessage(LocalizeUtils.getClientSideTranslation("message.raglok.future"));
                        }
                        break;
                    case BOMBS:
                        this.playSound(SoundRegistry.RAGLOK_RAIN.get(), 1.0F, 1.0F);
                        if (!level().isClientSide()) {
                            player.sendSystemMessage(LocalizeUtils.getClientSideTranslation("message.raglok.rain"));
                            player.sendSystemMessage(LocalizeUtils.getClientSideTranslation("message.raglok.kill"));
                        }
                        break;
                    case SLOW:
                        this.playSound(SoundRegistry.RAGLOK_NOTHING.get(), 1.0F, 1.0F);
                        if (!level().isClientSide()) {
                            player.sendSystemMessage(LocalizeUtils.getClientSideTranslation("message.raglok.compare"));
                        }
                        break;
                    default:
                        break;
    ...
    public void die(DamageSource source) {
        if (!avengeAbilityUsed) {
            if (!level().isClientSide()) {
                ...
                for (Player p : players) {
                    p.sendSystemMessage(LocalizeUtils.getClientSideTranslation("message.raglok.avenge"));
                }
            }
