...
public class EntityKarot extends EntityDivineBoss {
    ...
    public void tick() {
        if (this.spawnTick == 0 && !this.level().isClientSide()) {
            EntityRegistry.ANGRY_GLINTHOP.get().spawn((ServerLevel) level(), ItemStack.EMPTY, null, blockPosition(), EntitySpawnReason.MOB_SUMMONED, true, false);
            this.level().addParticle(ParticleTypes.ASH, getX(), getY() + 0.5D,
    ...
    public void die(DamageSource source) {
        ...
        super.kill(level().getServer().getLevel(level().dimension()));
    }
}