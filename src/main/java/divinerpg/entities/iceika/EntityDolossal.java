...
public class EntityDolossal extends AbstractHorse {
    private static final Supplier<Ingredient> FOOD = Suppliers.memoize(() ->
            Ingredient.of(BuiltInRegistries.ITEM.getOrThrow(TagRegistry.FOOD_DOLOSSAL))
    );
    ...
    @Override public void aiStep() {
        super.aiStep();
        if(getDeltaMovement().y < 0 && ((getControllingPassenger() instanceof LocalPlayer player && player.input.keyPresses.jump()) || (!hasControllingPassenger() && !onGround()))) {
    ...
    @Override public AgeableMob getBreedOffspring(ServerLevel level, AgeableMob mob) {
        EntityDolossal dol = (EntityDolossal) ENTITY_TYPE.get(Identifier.fromNamespaceAndPath(MODID, "dolossal")).get().value().create(level, EntitySpawnReason.TRIGGERED);
        if(dol != null) setOffspringAttributes(mob, dol);
        return dol;
    }
    @Override public boolean isFood(ItemStack stack) {return FOOD.get().test(stack);}
    //TODO: to improve (player doesn't swing arm, dolossals can't breed)
    @Override public InteractionResult mobInteract(Player player, InteractionHand hand) {
        ItemStack stack = player.getItemInHand(hand);
        if(FOOD.get().test(stack)) {
    ...
    @Override protected void playStepSound(BlockPos pos, BlockState stat) {
        playSound(SoundEvents.CHICKEN_STEP.value(), .25F, .6F);
    }
