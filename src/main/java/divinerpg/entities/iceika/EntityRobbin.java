...
public class EntityRobbin extends EntityDivineFlyingMonster {
    private static final TargetingConditions NON_COMBAT_TARGETING = TargetingConditions.forNonCombat().range(32.0D).ignoreLineOfSight();
    ...
    protected void readAdditionalSaveData(ValueInput input) {
        ...
        ItemStack item = input.read("MouthPiece", ItemStack.CODEC).orElse(ItemStack.EMPTY);
        setMouthPiece(item);
    }
    ...
    protected void addAdditionalSaveData(ValueOutput output) {
        ...
        ItemStack mouthItem = getItemBySlot(EquipmentSlot.MAINHAND);
        if (!mouthItem.isEmpty()) {
            output.store("MouthPiece", ItemStack.CODEC, mouthItem);
        }
    }
    ...
    protected void customServerAiStep(ServerLevel level) {
        ...
        if (path == null && tiredTicks % 10 == 0) {
            BlockPos pos;
            if (isTired()) {
                List<EntityKitra> whales = level().getEntitiesOfClass(EntityKitra.class, getBoundingBox().inflate(32.0));
                EntityKitra nearbyWhale = level.getNearestEntity(
                        whales,
                        NON_COMBAT_TARGETING,
                        this,
                ...
                if (nearbyWhale == null) {
                    for (int x = blockPosition().getX() - 5; x < blockPosition().getX() + 5; x++)
                        for (int y = blockPosition().getY() - 5; y < blockPosition().getY() + 5; y++)
                            for (int z = blockPosition().getZ() - 5; z < blockPosition().getZ() + 5; z++)
                                if (!level().isOutsideBuildHeight(pos = new BlockPos(x, y, z)) && level().getBlockState(pos).is(BlockRegistry.robbinHut.get())) {
                    ...
                    for (int x = blockPosition().getX() - 5; x < blockPosition().getX() + 5; x++)
                        for (int y = blockPosition().getY() - 5; y < blockPosition().getY() + 5; y++)
                            for (int z = blockPosition().getZ() - 5; z < blockPosition().getZ() + 5; z++)
                                if (!level().isOutsideBuildHeight(pos = new BlockPos(x, y, z)) && level().getBlockState(pos).isFaceSturdy(level(), pos, Direction.UP)) {
            ...
            } else if (wantsNest || !getItemBySlot(EquipmentSlot.MAINHAND).isEmpty()) {
                for (int x = blockPosition().getX() - 5; x < blockPosition().getX() + 5; x++)
                    for (int y = blockPosition().getY() - 5; y < blockPosition().getY() + 5; y++)
                        for (int z = blockPosition().getZ() - 5; z < blockPosition().getZ() + 5; z++)
                            if (!level().isOutsideBuildHeight(pos = new BlockPos(x, y, z)) && level().getBlockState(pos).is(BlockRegistry.robbinNest.get()) && level().getBlockEntity(pos) instanceof RobbinNestBlockEntity nest && (nest.isEmpty() || (wantsNest && !nest.getItem().is(ItemRegistry.robbin_egg.get()))))
                                getNavigation().moveTo(x, y, z, 1D);
            } else {
                List<ItemEntity> items = level().getEntitiesOfClass(ItemEntity.class, getBoundingBox().inflate(5.0));
                if (!items.isEmpty()) {
                    ItemEntity nearest = items.stream().min(Comparator.comparingDouble(this::distanceToSqr)).orElse(null);
                    if (nearest != null) {
    ...
    class RobbinRandomStrollGoal extends WaterAvoidingRandomStrollGoal {
        ...
        public boolean canUse() {
            return !isNoGravity() && getNavigation().getPath() == null && super.canUse();
        }

        @Override
        public boolean canContinueToUse() {
            return !isNoGravity() && super.canContinueToUse();
        }
    ...
    class FollowWhaleGoal extends Goal {
        ...
        public boolean canUse() {
            if (targetWhale == null || !targetWhale.isAlive()) {
                List<EntityKitra> whales = level().getEntitiesOfClass(EntityKitra.class, getBoundingBox().inflate(FOLLOW_DISTANCE));
                targetWhale = level().getServer().getLevel(level().dimension()).getNearestEntity(
                        whales,
                        NON_COMBAT_TARGETING,
                        EntityRobbin.this,
                        getX(), getY(), getZ()
    ...
    public class GoalRideWhaleWhenTired extends Goal {
        ...
        public boolean canUse() {
            if (isTired()) {
                List<EntityKitra> whales = level().getEntitiesOfClass(EntityKitra.class, getBoundingBox().inflate(16.0));
                whale = level().getServer().getLevel(level().dimension()).getNearestEntity(
                        whales,
                        NON_COMBAT_TARGETING,
                        EntityRobbin.this,
                        getX(), getY(), getZ()
