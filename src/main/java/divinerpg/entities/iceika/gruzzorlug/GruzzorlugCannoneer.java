...
public class GruzzorlugCannoneer extends Gruzzorlug implements RangedAttackMob {
	...
	@Override public void performRangedAttack(LivingEntity target, float f) {
		if(isAlive() && getTarget() != null) {
            ThrowableProjectile projectile = FROST_CANNON_SHOT.get().create(level());
	...
	@Override protected void updateTrades() {addOffersFromItemListings(getOffers(), GRUZZORLUG_CANNONEER.get(level(), getRandom()), 3);}
}