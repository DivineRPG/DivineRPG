...
public class GruzzorlugCommander extends Gruzzorlug implements RangedAttackMob {
	...
	@Override public void performRangedAttack(LivingEntity target, float f) {
		if(isAlive() && getTarget() != null) {
            ThrowableProjectile projectile = FRACTITE_CANNON_SHOT.get().create(level());
	...
	@Override protected void updateTrades() {addOffersFromItemListings(getOffers(), GRUZZORLUG_COMMANDER.get(level(), getRandom()), 3);}
}