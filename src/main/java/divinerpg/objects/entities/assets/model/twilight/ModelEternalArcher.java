...
public class ModelEternalArcher extends ItemModel {
...
//        armsRight[1].render(scale);
        ...
        // check if we really need to save that order
        Arrays.stream(armsLeft).forEach(x -> x.render(scale));
        Arrays.stream(armsRight).forEach(x -> x.render(scale));
    }
