...
public class RenderEdenChestItem extends ItemStackTileEntityRenderer implements Callable<ItemStackTileEntityRenderer> {
    public final ItemStackTileEntityRenderer instance;

    private final ModelEdenChest model = new ModelEdenChest();

    public RenderEdenChestItem() {
        instance = this;
    }
    ...
    public void renderByItem(ItemStack stack, ItemCameraTransforms.TransformType transformType, MatrixStack matrixStack, IRenderTypeBuffer buffer, int combinedLight, int combinedOverlay) {
        ...
        if (item == Item.byBlock(BlockRegistry.edenChest)) {
