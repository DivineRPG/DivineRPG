...
public class RenderPresentBoxItem extends ItemStackTileEntityRenderer implements Callable<ItemStackTileEntityRenderer> {
    public final ItemStackTileEntityRenderer instance;

    private final ModelPresentBox model = new ModelPresentBox();

    public RenderPresentBoxItem() {
        instance = this;
    }
    ...
    public void renderByItem(ItemStack stack, ItemCameraTransforms.TransformType transformType, MatrixStack matrixStack, IRenderTypeBuffer buffer, int combinedLight, int combinedOverlay) {
        ...
        if (item == Item.byBlock(BlockRegistry.presentBox)) {
