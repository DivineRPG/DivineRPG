...
public class ModelMadivel<S extends MadivelRenderState> extends EntityModel<S> {
    ...
    public ModelMadivel(ModelPart root) {
        super(root);
    ...
    public void setupAnim(S state) {
        super.setupAnim(state); // Crucial: Resets model part positions per-frame
        ...
        float rightSunOrbitAngle = ageInTicks * orbitSpeed * .8F;
        ...
        float attackAnim = state.attackTime;
        if (attackAnim > 0.0F) {
            rightArm.xRot = leftArm.xRot = -1.5F + 1.5F * Mth.triangleWave(attackAnim * 10.0F - ageInTicks, 10);
        } else {
