...
public class ModelGreenfeet<S extends GreenfeetRenderState> extends EntityModel<S> {
    ...
    public ModelGreenfeet(ModelPart root) {
        super(root);
    ...
    public void setupAnim(S state) {
        super.setupAnim(state); // Crucial: Resets pose parameters safely for the render state pipeline
        ...
        if (state.attackTime > 0.0F) {
            setupAttackAnimation(state.attackTime, ageInTicks);
        }
    }
    ...
    protected void setupAttackAnimation(float attackTime, float ageInTicks) {
        float f = attackTime;
        ...
        RightArm.zRot += Mth.sin(attackTime * Mth.PI) * -.4F;
    }
}