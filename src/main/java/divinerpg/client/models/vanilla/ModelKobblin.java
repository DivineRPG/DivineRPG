...
public class ModelKobblin<S extends LivingEntityRenderState> extends EntityModel<S> {
    ...
    public int tempBiomeColor = 0xFF00;

    public ModelKobblin(ModelPart root) {
        super(root);
    ...
    public void setupAnim(S state) {
        ...
        if (state.hasPose(Pose.EMERGING)) {
        ...
        } else if (state.hasPose(Pose.SHOOTING)) {
            ...
            GrassOverlay.yRot = Head.yRot = netHeadYaw / 2 + fs;
            GrassOverlay.xRot = Head.xRot = headPitch / 2 + f1s;
            Body.xRot = 0;
