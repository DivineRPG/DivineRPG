...
public class JungleTempleChestModifier extends LootModifier {
    ...
    protected ObjectArrayList<ItemStack> doApply(ObjectArrayList<ItemStack> generatedLoot, LootContext context) {
        if (context.getRandom().nextFloat() <= 0.3){
            ItemStack toAdd = new ItemStack(BuiltInRegistries.ITEM.get(new ResourceLocation(DivineRPG.MODID, "jungle_shards")), 1 + context.getRandom().nextInt(3));
            generatedLoot.add(toAdd);
        }
        if (context.getRandom().nextFloat() <= 0.5){
            ItemStack toAdd = new ItemStack(BuiltInRegistries.BLOCK.get(new ResourceLocation(DivineRPG.MODID, "jungle_spider_pumpkin")), 1 + context.getRandom().nextInt(1));
            generatedLoot.add(toAdd);
        }
