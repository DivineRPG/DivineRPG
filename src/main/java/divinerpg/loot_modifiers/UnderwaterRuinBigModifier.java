...
import static divinerpg.registries.ItemRegistry.aquatic_coating_template;
...
public class UnderwaterRuinBigModifier extends LootModifier {
    ...
    @Override protected ObjectArrayList<ItemStack> doApply(ObjectArrayList<ItemStack> generatedLoot, LootContext context) {
        if(context.getRandom().nextFloat() <= .4) generatedLoot.add(new ItemStack(aquatic_coating_template.get(), 1));
