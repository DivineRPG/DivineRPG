...
public class AbandonedMineshaftChestModifier extends LootModifier {
    ...
    protected ObjectArrayList<ItemStack> doApply(ObjectArrayList<ItemStack> generatedLoot, LootContext context) {
        if (context.getRandom().nextFloat() <= 0.5){
            ItemStack toAdd = new ItemStack(BuiltInRegistries.ITEM.get(new ResourceLocation(DivineRPG.MODID, "realmite_pickaxe")), 1);
            generatedLoot.add(toAdd);
        }
        if (context.getRandom().nextFloat() <= 0.3){
            ItemStack toAdd = new ItemStack(BuiltInRegistries.BLOCK.get(new ResourceLocation(DivineRPG.MODID, "skeleton_torch")), 4 + context.getRandom().nextInt(4));
            generatedLoot.add(toAdd);
        }
