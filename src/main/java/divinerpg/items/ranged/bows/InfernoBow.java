...
import static divinerpg.utils.RarityList.INFERNO;
import static divinerpg.utils.ToolStats.INFERNO_BOW;
...
public class InfernoBow extends ItemBow {
    public InfernoBow(Properties properties) {super(INFERNO_BOW, properties.fireResistant().component(DataComponentRegistry.weaponAbility, 0).component(DataComponentRegistry.weaponPower, 1F), inferno_arrow, INFERNO);}
    ...
    @Override public boolean releaseUsing(ItemStack stack, Level level, LivingEntity living, int timeLeft) {
        int i;
    ...
    public void inventoryTick(ItemStack stack, ServerLevel level, Entity owner, @Nullable EquipmentSlot slot) {
        if((level.getGameTime() & 1L) == 0 && level instanceof ServerLevel server) {
