...
public class SkyDropStaff extends ItemRangedWeapon {
    ...
    public SkyDropStaff(Properties properties, Supplier<EntityType<? extends Projectile>> projectileType, int arcana, int damage, int count, int delay, int uses) {
        super(properties.durability(uses), projectileType);
    ...
    @Override protected void shootProjectile(LivingEntity shooter, Projectile projectile, int i, float velocity, float inaccuracy, float angle, @Nullable LivingEntity livingEntity1) {
        ...
        projectile.moveOrInterpolateTo(new Vec3(pos.getBlockPos().getX() + shooter.getRandom().nextDouble() - .5, pos.getBlockPos().getY() + 20, pos.getBlockPos().getZ() + shooter.getRandom().nextDouble() - .5));
    }
}