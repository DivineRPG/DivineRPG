...
import static divinerpg.utils.RarityList.RED;
...
public class ItemBossSpawner extends ItemMod {
    ...
    public ItemBossSpawner(String key, ResourceKey<Level> dimension, Supplier<EntityType<?>> entity, Properties properties) {
    ...
    public ItemBossSpawner(String key, ResourceKey<Level> dimension, Properties properties) {
        super(properties.stacksTo(1));
        dimensionID = dimension;
        langKey = key;
        ent = null;
    ...
    @Override public InteractionResult useOn(UseOnContext context) {
        ...
        if(world.dimension() != dimensionID) {
            player.sendSystemMessage(LocalizeUtils.clientMessage(AQUA, langKey));
            return InteractionResult.FAIL;
        } else if(world.getDifficulty() == PEACEFUL) {
            player.sendSystemMessage(LocalizeUtils.clientMessage(AQUA, "boss.peaceful"));
            return InteractionResult.FAIL;
            ...
            player.sendSystemMessage(LocalizeUtils.clientMessage(AQUA, "boss.space"));
            return InteractionResult.FAIL;
        } else {
