	...
	public static void init(LivingSpawnEvent event) {
		...
		if (entity.world.getGameRules().getBoolean("doMobSpawning")) {
			if (entity instanceof EntityWaterMob) {
				if (entity.world.getDifficulty() != EnumDifficulty.PEACEFUL && !entity.world.isRemote
					...
					if (entity.getRNG().nextInt(2500) == 1 && entity.world.loadedEntityList.size() < 90) {
					...
					if (entity.getRNG().nextInt(5000) == 1 && entity.world.loadedEntityList.size() < 90) {
					...
					if (entity.world.getBiome(entity.getPosition()) == Biomes.DEEP_OCEAN
					        && entity.getRNG().nextInt(10000) == 1 && entity.world.loadedEntityList.size() < 90) {
