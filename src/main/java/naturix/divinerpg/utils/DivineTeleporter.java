								...
								for (int l4 = -1; l4 < 4; ++l4) {
									...
									if (l4 < 0
									        && !worldServerInstance.getBlockState(blockpos$mutableblockpos)
									                .getMaterial().isSolid()
									        || l4 >= 0 && !worldServerInstance.isAirBlock(blockpos$mutableblockpos)) {
										continue label142;
									}
								}
							}
			...
			for (int j6 = l - i; j6 <= l + i; ++j6) {
				...
				for (int i7 = worldServerInstance.getActualHeight() - 1; i7 >= 0; --i7) {
					if (worldServerInstance.isAirBlock(blockpos$mutableblockpos.setPos(l5, i7, j6))) {
						while (i7 > 0
						        && worldServerInstance.isAirBlock(blockpos$mutableblockpos.setPos(l5, i7 - 1, j6))) {
							--i7;
						}
							...
							for (int j10 = 0; j10 < 4; ++j10) {
								for (int j11 = -1; j11 < 4; ++j11) {
									...
									if (j11 < 0
									        && !worldServerInstance.getBlockState(blockpos$mutableblockpos)
									                .getMaterial().isSolid()
									        || j11 >= 0 && !worldServerInstance.isAirBlock(blockpos$mutableblockpos)) {
										continue label562;
									}
								}
							}
	...
	if (destinationCoordinateCache.containsKey(k)) {
		...
		for (int l = -128; l <= 128; ++l) {
			...
			for (int i1 = -128; i1 <= 128; ++i1) {
				for (BlockPos blockpos = blockpos4.add(l, worldServerInstance.getActualHeight() - 1 - blockpos4.getY(),
