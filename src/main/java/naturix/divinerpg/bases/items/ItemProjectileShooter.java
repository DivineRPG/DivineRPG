	...
	public ActionResult<ItemStack> onItemRightClick(World world, EntityPlayer player, EnumHand handIn) {
		...
		if (Ticker.tick >= stack.getTagCompound().getLong("CanShootTime")) {
			if (player.capabilities.isCreativeMode || this.ammo == null
				...
				if (!world.isRemote) {
					// world.playSoundAtEntity(player, this.soundName != null ? this.soundName :
					// "random.bow", 1, 1);
					// EntityThrowable bullet = this.hasParticle
					// ? (this.projectileTex != null
					// ? new EntityParticleBullet(world, player, this.damage, this.projectileTex,
					// this.fx)
					// : new EntityParticleBullet(world, player, this.damage, this.ammo, this.fx))
					// : (this.projectileTex != null
					// ? new EntityShooterBullet(world, player, this.damage, this.projectileTex)
					// : new EntityShooterBullet(world, player, this.damage, this.ammo));
