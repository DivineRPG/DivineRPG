...
import naturix.divinerpg.particle.EntitySkythernPortalFX;
...
public class SkythernPortal extends BlockBreakable {
	...
	public void onEntityCollidedWithBlock(World worldIn, BlockPos pos, IBlockState state, Entity entity) {
		if ((entity.getRidingEntity() == null) && ((entity instanceof EntityPlayerMP))) {
			...
			Block blockFrame = ModBlocks.blockApalachia;
			
			if(thePlayer.timeUntilPortal > 0) 
				thePlayer.timeUntilPortal = 10;
			else if(thePlayer.dimension != dimensionID) {
				thePlayer.timeUntilPortal = 10;
				thePlayer.mcServer.getPlayerList().transferPlayerToDimension(thePlayer, dimensionID, new ModTeleporterSkythern(thePlayer.mcServer.getWorld(dimensionID), dimensionID, this));
			} else {
				thePlayer.timeUntilPortal = 10;
				thePlayer.mcServer.getPlayerList().transferPlayerToDimension(thePlayer, 0, new ModTeleporterSkythern(thePlayer.mcServer.getWorld(0), 0, this));
			}
		}
	}
	...
	public void randomDisplayTick(IBlockState state, World worldIn, BlockPos pos, Random rand) {
		...
		for(int i = 0; i < 4; ++i) {
			...
			EntitySkythernPortalFX var20 = new EntitySkythernPortalFX(worldIn, d0, d1, d2, d3, d4, d5);
	...
	public void neighborChanged(IBlockState state, World worldIn, BlockPos pos, Block blockIn, BlockPos fromPos)
    {
        ...
        {
            SkythernPortal.Size EdenBlock$size = new SkythernPortal.Size(worldIn, pos, EnumFacing.Axis.X);
        ...
        {
            SkythernPortal.Size EdenBlock$size1 = new SkythernPortal.Size(worldIn, pos, EnumFacing.Axis.Z);
	...
	public static class Size {
		...
		protected int getDistanceUntilEdge(BlockPos pos, EnumFacing facing) {
			...
			for(i = 0; i < 22; ++i) {
				...
				if(!this.isEmptyBlock(this.world.getBlockState(blockpos).getBlock()) || this.world.getBlockState(blockpos.down()).getBlock() != ModBlocks.blockApalachia) {
					break;
				}
			}
			...
			label56:
				...
				for(this.height = 0; this.height < 21; ++this.height) {
					for(int i = 0; i < this.width; ++i) {
						...
						if(block == ModBlocks.portalSkythern) {
						...
						if(i == 0) {
							...
							if(block != ModBlocks.blockApalachia) {
								break label56;
							}
						}
		...
		for(int j = 0; j < this.width; ++j) {
			if (this.world.getBlockState(this.bottomLeft.offset(this.rightDir, j).up(this.height)).getBlock() != ModBlocks.blockApalachia) {
				this.height = 0;
				break;
			}
		}
		...
		protected boolean isEmptyBlock(Block blockIn) {
			return blockIn.getMaterial(blockIn.getDefaultState()) == Material.AIR || blockIn == ModBlocks.blueFire || blockIn == ModBlocks.portalSkythern;
		}
		...
		public void placePortalBlocks() {
			for(int i = 0; i < this.width; ++i) {
				...
				for(int j = 0; j < this.height; ++j) {
					this.world.setBlockState(blockpos.up(j), ModBlocks.portalSkythern.getDefaultState().withProperty(SkythernPortal.AXIS, this.axis), 2);
				}
			}
		}
	}
