...
public class ItemArmorBase extends net.minecraft.item.ItemArmor implements ISpecialArmor, IHasModel {
    ...
    public ItemArmorBase(EnumArmor armorMaterial, EntityEquipmentSlot slot, Object[] info, boolean vethean, String helmType) {
    ...
    public ItemArmorBase(EnumArmor armorMaterial, EntityEquipmentSlot slot, String name) {
        this(armorMaterial, slot, armorMaterial.getDamageReduction(), name, new Object[] { "null", "null" }, false, null);
    }
    
    public ItemArmorBase(EnumArmor armorMaterial, EntityEquipmentSlot slot, String name, boolean vethean) {
        this(armorMaterial, slot, armorMaterial.getDamageReduction(), name, new Object[] { "null", "null" }, vethean, null);
    }
    ...
    public ItemArmorBase(EnumArmor armorMaterial, EntityEquipmentSlot slot, String name, Object[] info, boolean vethean, String helmType) {
        this(armorMaterial, slot, armorMaterial.getDamageReduction(), name, info, vethean, helmType);
    }

    public ItemArmorBase(EnumArmor armorMaterial, EntityEquipmentSlot slot, int dR, String name, Object[] info, boolean vethean, String helmType) {
        super(armorMaterial.getArmorMaterial(), 0, slot);
        this.armorMaterial = armorMaterial;
        this.fullReduction = dR;
        this.armorInfo = info;

        if (slot == EntityEquipmentSlot.HEAD) damageReduction = ((((double) fullReduction) / 24) * 5) / 100;
        else if (slot == EntityEquipmentSlot.CHEST) damageReduction = ((((double) fullReduction) / 24) * 8) / 100;
        else if (slot == EntityEquipmentSlot.LEGS) damageReduction = ((((double) fullReduction) / 24) * 7) / 100;
        else if (slot == EntityEquipmentSlot.FEET) damageReduction = ((((double) fullReduction) / 24) * 4) / 100;
        ...
        for (int i = 0; i < armorInfo.length; i++) {
            ...
            if (strInfo.contains("#")) {
                try {
                    ...
                    DivineRPG.logger.error("Attempted to replace a token with the value at the index before it, but that index does not exist!! "
