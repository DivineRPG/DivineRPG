...
public class Aquamarine extends ItemBucket implements IHasModel {
			...
			if (!worldIn.isBlockModifiable(playerIn, blockpos)) {
				...
				if (!playerIn.canPlayerEdit(blockpos1, raytraceresult.sideHit, itemstack)) {
					...
					return !playerIn.capabilities.isCreativeMode
					        ? new ActionResult(EnumActionResult.SUCCESS, new ItemStack(/** ModItems.aquamarine */
		...
		if (this.containedBlock == Blocks.AIR) {
			...
			if (!worldIn.isAirBlock(posIn) && !flag && !flag1) {
				...
				if (worldIn.provider.doesWaterVaporize() && this.containedBlock == Blocks.FLOWING_WATER) {
					...
					for (int k = 0; k < 8; ++k) {
						worldIn.spawnParticle(EnumParticleTypes.SMOKE_LARGE, l + Math.random(), i + Math.random(),
						        j + Math.random(), 0.0D, 0.0D, 0.0D);
					}
				} else {
					...
					worldIn.updateBlockTick(posIn, Blocks.WATER, 1, 1);
				}
