 ...
 * /01/2019
 */
public class TileEntityFrostedChest extends TileEntityLockableLoot implements ITickable {
    ...
    public Container createContainer(InventoryPlayer playerInventory, EntityPlayer playerIn) {
        return new ContainerFrostedChest(playerInventory, this, playerIn);
    }
    ...
    public void update()
    {
        if (!this.world.isRemote && this.numPlayersUsing != 0 && (this.ticksSinceSync + pos.getX() + pos.getY() + pos.getZ()) % 200 == 0)
        {
            ...
            {
                if (entityplayer.openContainer instanceof ContainerFrostedChest)
                {
                    if (((ContainerFrostedChest)entityplayer.openContainer).getChestInventory() == this)
                    {
