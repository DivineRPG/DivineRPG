...
public class AltarOfCorruptionGUI extends GuiContainer {
    ...
    private static final ResourceLocation ENCHANTMENT_TABLE_BOOK_TEXTURE = new ResourceLocation(
            "divinerpg:textures/model/altar_book.png");
    /**
    ...
    private final ContainerAltarOfCorruption container;
    public int ticks;
    public float flip;
    public float oFlip;
    public float flipT;
    ...
    public AltarOfCorruptionGUI(InventoryPlayer inventory, World worldIn, IWorldNameable nameable) {
        super(new ContainerAltarOfCorruption(inventory, worldIn));
        this.playerInventory = inventory;
        this.container = (ContainerAltarOfCorruption) this.inventorySlots;
    ...
    protected void drawGuiContainerForegroundLayer(int mouseX, int mouseY) {
        String title = this.nameable.getDisplayName().getUnformattedText();
        this.fontRenderer.drawString(title, (this.xSize / 2 - this.fontRenderer.getStringWidth(title) / 2), 4, 4210752);
        //this.fontRenderer.drawString(, 12, 5, 4210752);
        ...
        for (int l = 0; l < 3; ++l) {
            ...
            if (k1 == 0) {
                ...
                if (((k < l + 1 || this.mc.player.experienceLevel < k1) && !this.mc.player.capabilities.isCreativeMode)
                        || this.container.enchantClue[l] == -1) // Forge: render buttons as disabled when enchantable but enchantability not met on lower levels
                {
                    this.drawTexturedModalRect(i1, j + 14 + 19 * l, 0, 185, 108, 19);
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        for (int j = 0; j < 3; ++j) {
            ...
            if (this.isPointInRegion(60, 14 + 19 * j, 108, 17, mouseX, mouseY) && k > 0) {
                ...
                list.add("" + TextFormatting.WHITE + TextFormatting.ITALIC + I18n.format("container.enchant.clue",
                        enchantment == null ? "" : enchantment.getTranslatedName(l)));

                if (enchantment == null)
                    java.util.Collections.addAll(list, "",
