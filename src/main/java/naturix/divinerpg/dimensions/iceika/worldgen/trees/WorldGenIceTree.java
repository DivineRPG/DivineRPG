...
public class WorldGenIceTree extends WorldGenerator {
    IBlockState leaves = ModBlocks.brittleLeaves.getDefaultState().withProperty(BlockLeaves.DECAYABLE, false)
    ...
    public boolean generate(World world, Random rand, int i, int j, int k) {
        ...
        if (j < 1 || j + height + 1 > 256 || world.getBlockState(pos).getBlock() != ModBlocks.frozenGrass
                || j >= 256 - height - 1)
            return false;
            ...
            for (int x = i - k1; x <= i + k1; x++) {
                for (int z = k - k1; z <= k + k1; z++) {
                    ...
                    if (world.getBlockState(bPos).getBlock() != Blocks.AIR
                            && !world.getBlockState(bPos).getBlock().isLeaves(world.getBlockState(bPos), world, bPos))
                        return false;
                }
            }
        }
        setBlockAndNotifyAdequately(world, new BlockPos(i, j - 1, k), ModBlocks.frozenGrass.getDefaultState());
        setBlockAndNotifyAdequately(world, new BlockPos(i - 1, j - 1, k), ModBlocks.frozenGrass.getDefaultState());
        setBlockAndNotifyAdequately(world, new BlockPos(i, j - 1, k - 1), ModBlocks.frozenGrass.getDefaultState());
        setBlockAndNotifyAdequately(world, new BlockPos(i - 1, j - 1, k - 1), ModBlocks.frozenGrass.getDefaultState());
            ...
            for (int i4 = i - l1; i4 <= i + l1; i4++) {
                ...
                for (int l4 = k - l1; l4 <= k + l1; l4++) {
                    ...
                    if ((Math.abs(k4) != l1 || Math.abs(i5) != l1 || l1 <= 0)
                            && (world.getBlockState(new BlockPos(i4, k3, l4)).getBlock() == Blocks.AIR
                                    || world.getBlockState(new BlockPos(i4, k3, l4)).getBlock().canBeReplacedByLeaves(
                                            world.getBlockState(new BlockPos(i4, k3, l4)), world,
                                            new BlockPos(i4, k3, l4)))) {
        ...
        for (int l3 = 0; l3 < height - j3; l3++) {
            if (world.getBlockState(new BlockPos(i, j + l3, k)).getBlock() == Blocks.AIR
                ...
                setBlockAndNotifyAdequately(world, new BlockPos(i, j + l3, k), ModBlocks.frozenWood.getDefaultState());
                setBlockAndNotifyAdequately(world, new BlockPos(i - 1, j + l3, k),
                        ModBlocks.frozenWood.getDefaultState());
