...
public class ModTeleporterIceika extends Teleporter {
    ...
    protected final Long2ObjectMap<ModTeleporterIceika.PortalPosition> destinationCoordinateCache = new Long2ObjectOpenHashMap<ModTeleporterIceika.PortalPosition>(4096);
	protected final List destinationCoordinateKeys = new ArrayList();
	protected int dimensionID;
	protected Block portal, frameBlock;

	public ModTeleporterIceika(WorldServer worldServer, int dimensionID, Block portal, Block frameBlock) {
		super(worldServer);
	...
	public boolean placeInExistingPortal(Entity entityIn, float par2) {
		...
		if (this.destinationCoordinateCache.containsKey(k)) {
			ModTeleporterIceika.PortalPosition portalposition = (ModTeleporterIceika.PortalPosition) this.destinationCoordinateCache.get(k);
			d0 = 0.0D;
		...
		if (d0 >= 0.0D) {
			if (flag1) {
				this.destinationCoordinateCache.put(k, new ModTeleporterIceika.PortalPosition(object, this.myWorld.getTotalWorldTime()));
			}
				...
				for (j3 = 1; j3 < 3; ++j3) {
					for (k3 = -1; k3 < 3; ++k3) {
						...
						this.myWorld.setBlockState(new BlockPos(l3, i4, j4),
								flag ? Blocks.SNOW.getDefaultState() : Blocks.AIR.getDefaultState());
					}
				}
			}
		}
		...
		for (j3 = 0; j3 < 4; ++j3) {
			for (k3 = 0; k3 < 4; ++k3) {
				for (l3 = -1; l3 < 4; ++l3) {
					...
					this.myWorld.setBlockState(new BlockPos(i4, j4, k4), flag1 ? Blocks.SNOW.getDefaultState() : iblockstate,
							2);
				}
			}
