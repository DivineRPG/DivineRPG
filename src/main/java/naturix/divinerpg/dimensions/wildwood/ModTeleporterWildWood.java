...
public class ModTeleporterWildWood extends Teleporter {
...
//	This is where i was going to add an easter egg for Techno Kitty but they teased Evancaw so they lost that easter egg!
	...
	public boolean placeInExistingPortal(Entity entityIn, float par2) {
		...
		if (this.destinationCoordinateCache.containsKey(k)) {
			ModTeleporterWildWood.PortalPosition portalposition = (ModTeleporterWildWood.PortalPosition) this.destinationCoordinateCache.get(k);
			d0 = 0.0D;
		...
		if (d0 >= 0.0D) {
			if (flag1) {
				this.destinationCoordinateCache.put(k, new ModTeleporterWildWood.PortalPosition(object, this.myWorld.getTotalWorldTime()));
			}
		...
		for (j3 = 0; j3 < 4; ++j3) {
			for (k3 = 0; k3 < 4; ++k3) {
				for (l3 = -1; l3 < 4; ++l3) {
					...
					this.myWorld.setBlockState(new BlockPos(i4, j4, k4), flag1 ? ModBlocks.blockEden.getDefaultState() : iblockstate,
							2);
				}
			}
