...
import naturix.divinerpg.utils.ChunkCoordIntPair;
import naturix.divinerpg.utils.LongHashMap;
...
public class DivineTeleporter extends Teleporter {
    ...
    public void placeInPortal(Entity entity, double x, double y, double z, float par8) {
        if (this.worldServerInstance.provider.getDimension() == dimensionID) {
    ...
    public boolean placeInExistingPortal(Entity entity, double x, double y, double z, float rotation) {
        ...
        long j1 = ChunkCoordIntPair.a(i1, l);
        boolean flag = true;
        double d7;
        int l3;

        if (this.destinationCoordinateCache.contains(j1)) {
            PortalPosition portalposition = (PortalPosition) this.destinationCoordinateCache.getEntry(i1);
            d3 = 0.0D;
