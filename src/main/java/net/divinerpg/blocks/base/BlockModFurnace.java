...
public abstract class BlockModFurnace extends BlockContainer {
	...
	public void onBlockAdded(World w, int x, int y, int z) {
		...
		if(!w.isRemote) {
			...
			if(block.isOpaqueCube() && !block1.isOpaqueCube()) meta = 3;
			if(block1.isOpaqueCube() && !block.isOpaqueCube()) meta = 2;
			if(block2.isOpaqueCube() && !block3.isOpaqueCube()) meta = 5;
			if(block3.isOpaqueCube() && !block2.isOpaqueCube()) meta = 4;
