package net.divinerpg.world.feature;
...
public class DivineTree extends Feature<TreeConfig> {
    ...
    public boolean hasSpace(BlockState state) {
        return state.isAir() || state.is(BlockTags.LEAVES) || state.is(BlockTags.FLOWERS) || state.is(BlockTags.create(Identifier.fromNamespaceAndPath(MODID, "replaceable_by_trees/iceika"))) && !state.is(Blocks.WATER);
    }
    ...
    protected boolean defaultGrowOn(BlockState state) {return state.is(BlockTags.DIRT) || state.is(Blocks.GRASS_BLOCK);}
